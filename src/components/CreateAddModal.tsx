...
export function CreateAdModal() {
    ...
    async function handleCreatrAd(event: FormEvent) {
    ...
    return (
        <Dialog.Portal>
            ...
            <Dialog.Content className="fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25" >
                ...
                <form onSubmit={handleCreatrAd} className='mt-8 flex flex-col gap-4'>
                    <div className='flex flex-col gap-2'>
                        <label htmlFor="game" className='font-semibold'>Qual o game?</label>
                        <select name='game' id='game' className='bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 appearance-none' defaultValue="Selecione o game que deseja jogar">
                    ...
                    <label className='mt-2 flex gap-2 text-sm items-center'>
                        <Checkbox.Root className='w-6 h-6 rounded bg-zinc-900 p-1' checked={useVoiceChannel} onCheckedChange={(checked) => {
                            if (checked === true) {
